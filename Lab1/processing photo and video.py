...
def show_photo():
    ...
    img = cv2.imread(r'Lab1\car.jpg', 0)  # Загружает изображение в градациях серого (черно-белое).
    cv2.imshow('Display window', img)
    cv2.waitKey(0)

    cv2.namedWindow('Display window', cv2.WINDOW_NORMAL)  # Позволяет изменять размер окна вручную, фото растягивается
    img = cv2.imread(r'Lab1\car.jpg',-1)  # Загружает изображение с сохранением всех каналов, включая альфа-канал, если он есть.
    cv2.imshow('Display window', img)
    cv2.waitKey(0)
    ...
    img = cv2.imread(r'Lab1\car.jpg',1)  # Загружает цветное изображение по умолчанию. При этом все цветные каналы (BGR) сохраняются, но альфа-канал (прозрачность) игнорируется.
    cv2.imshow('Display window', img)
    cv2.waitKey(0)
...
def rgb_to_hsv():
    img = cv2.imread(r'Lab1\car.jpg', 1)
...
def stream_video_from_phone():
    video = cv2.VideoCapture("https://172.20.10.3:8080/video")
